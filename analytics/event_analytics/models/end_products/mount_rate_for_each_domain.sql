WITH
    tagload_counts
        AS (
        --Count total tagloads for each domain
        SELECT
            domain
...
--             trendii_de_assessment.grr_dev.stg_event_tag_loaded
  ...
  , mount_counts
        AS (
        --Count total mounts for each domain
        SELECT
            domain
          , COUNT(DISTINCT event_id) AS total_mounts
        FROM
            {{ ref('stg_event_mounts') }}
--             trendii_de_assessment.grr_dev.stg_event_mounts
  ...
  , final
        AS (
--  Join and calculate the mount rate, rounded to 2 decimal places
        SELECT
            t.domain
          , t.total_tagloads
